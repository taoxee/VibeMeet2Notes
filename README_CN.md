...
- 🔌 **多供应商支持** — 17 家 ASR/LLM 供应商，自由组合
- 💾 **凭证本地存储** — API Key 仅保存在浏览器 localStorage，不经过服务端
- ⚡ **缓存检测** — 相同文件 + 相同供应商 + 相同模型 + 相同提示词自动复用历史结果，提示词变更仅重跑 LLM
...
- 🗂️ **模板浏览器** — 紧凑栏 + 内联展开面板，按领域（会议/销售/HR/产品/学习/法律）浏览 13 个内置模板；常用模板一键应用，预览后选用；支持保存自定义模板
- ✏️ **自定义提示词** — 展开编辑器直接修改 LLM 系统提示词，自动保存到浏览器，一键恢复默认
- 📝 **可编辑纪要** — 处理完成后可直接在页面修改生成的会议纪要
...
4. 从模板浏览器选择模板，或（可选）展开「自定义提示词」直接编辑提示词
...
├── data/               # 数据文件
│   ├── custom-prompts/
│   │   ├── builtin-templates.json  # 内置模板（含域名/常用/描述/内容）
│   │   └── user-templates.json     # 用户自定义模板（本地，不提交）
│   ├── CHANGELOG.md
│   └── vendor_keys.csv
